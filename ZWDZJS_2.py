...
card[a_3] = pygame.image.load('wandou_2_card.gif').convert_alpha()
...
imageDict[a_3] = pygame.image.load('wandou_2.gif').convert_alpha()
...
nums = {}
nums[a_0] = [10,2,80,80,12]
nums[a_1] = [5,2,80,80,7]
nums[a_2] = [3,2,68,80,12]
nums[a_3] = [5,2,80,80,7]
...
image[a_3] = [imageDict[a_3].subsurface(Rect((i*nums[a_3][2],0),(nums[a_3][2],nums[a_3][3])))
                        for i in xrange(nums[a_3][4])]
...
class Choose:
    ...
    def mainloop(self):
        ...
        while b:
            ...
            for j in xrange(4):
                screen.blit(card[str(j)], (25+53*j,90))
            for event in pygame.event.get():
                ...
                for j in xrange(4):  
                    if event.type == pygame.MOUSEBUTTONDOWN and 25+53*j<x<25+53*(j+1) and 90<y<165:
                        card[i] = card[str(j)]
                        pl[i] = str(j)
                        i = i + 1
            while a < i:
...
class Game:
    def __init__(self):
        pass
    def panduan(self,x,y):
        row = [90,180,280,380,475,575]
        row_1 = [150,250,350,450,550]
        line = [40,110,190,280,350,440,515,590,675,750]
        line_1 = [72,152,230,315,395,480,555,630,720]
        for j in xrange(5):
            if row[j] <= y < row[j+1]:
                y = row_1[j]
        for j in xrange(9):
            if line[j] <= x < line[j+1]:
                x = line_1[j]
        if x < 40 or x >= 750 or y < 90 or y >= 575:
            x = 1000
            y = 0
        return x,y
    def mainloop(self):
                    ...
                    while tru:
                        ...
                        for event in pygame.event.get():
                            if event.type == pygame.MOUSEBUTTONDOWN:
                                x,y = pygame.mouse.get_pos()
                                g0 = Game()
                                x,y = g0.panduan(x,y+nums[pl[i]][3]/2)
                                r_Pla = Rect(x-nums[pl[i]][2]/2,y-nums[pl[i]][3],nums[pl[i]][2],nums[pl[i]][3])
                                Plan[i_0] = Plants(image[pl[i]],r_Pla,i,x-nums[pl[i]][2]/2,y-nums[pl[i]][3])
                                pla[i_0] = [image[pl[i]][0], x-nums[pl[i]][2]/2, y-nums[pl[i]][3]]
                                i_0 = i_0 + 1
                                tru = False
                    i = i + 1                               
            screen.blit(background_1, (0,0))#将背景图画上去
